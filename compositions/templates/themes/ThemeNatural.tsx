...
export const ThemeNaturalTexts = [
  "ORGANIC • NATURAL • PURE",
  "Back to",
  "Nature"
] as const;

export const ThemeNatural = ({ startDelay = 0, texts }: {
  startDelay?: number;
  /** Replaces the on-screen text, slot by slot (see ThemeNaturalTexts). */
  texts?: readonly string[];
}) => {
  const t = mergeTexts(texts, ThemeNaturalTexts);
    ...
    <AbsoluteFill style={{ background: "#f5f1eb" }}>
      ...
      >
        ...
        >
          {t[1]}
          <br />
          <span style={{ fontWeight: 600, color: "#5a6b4d" }}>{t[2]}</span>
        </div>
      </div>
