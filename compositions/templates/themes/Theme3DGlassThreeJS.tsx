...
import { AbsoluteFill, useCurrentFrame, useVideoConfig, spring, useDelayRender } from "remotion";
import { ThreeCanvas } from "@remotion/three";
import { useThree } from "@react-three/fiber";
import * as THREE from "three";
import { MeshTransmissionMaterial, Environment, Text } from "@react-three/drei";
import { font, mergeTexts } from "./common";

/** Default on-screen text slots, in reading order. */
export const Theme3DGlassThreeJSTexts = [
  "PREMIUM",
  "Glass",
  "THREE.JS 3D GLASS"
] as const;

/** Holds the Remotion render while the environment map / fonts suspend (added for this project). */
const WaitForSuspense = () => {
  const { delayRender, continueRender } = useDelayRender();
  const [handle] = useState(() => delayRender("Loading three.js glass scene"));
...
const BackgroundElements = ({ frame, t }: { frame: number; t: readonly string[] }) => {
  ...
  return (
    <>
      ...
      <Text
        ...
        anchorX="center"
        anchorY="middle"
        // drei/troika Text cannot parse woff2, so these point at woff builds of Inter.
      ...
      <Text
        ...
        anchorX="center"
        anchorY="middle"
        font="https://cdn.jsdelivr.net/npm/@fontsource/inter@5/files/inter-latin-700-normal.woff"
        fontWeight={700}
      >
        {t[1]}
      </Text>
...
// メインシーン
const GlassScene3D = ({ frame, progress, t }: {
  frame: number;
  progress: number;
  t: readonly string[];
}) => {
  ...
  return (
    <>
      ...
      {/* 背景要素（ガラス越しに見える） */}
      <BackgroundElements frame={frame} t={t} />
...
export const Theme3DGlassThreeJS = ({ startDelay = 0, texts }: {
  startDelay?: number;
  /** Replaces the on-screen text, slot by slot (see Theme3DGlassThreeJSTexts). */
  texts?: readonly string[];
}) => {
  const t = mergeTexts(texts, Theme3DGlassThreeJSTexts);
  ...
  return (
    ...
    >
      ...
      >
        <Suspense fallback={<WaitForSuspense />}>
          <GlassScene3D frame={frame - startDelay} progress={progress} t={t} />
        </Suspense>
      </ThreeCanvas>
