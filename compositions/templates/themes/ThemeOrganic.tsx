...
export const ThemeOrganicTexts = [
  "FLUID FORMS",
  "Organic",
  "Shapes"
] as const;

export const ThemeOrganic = ({ startDelay = 0, texts }: {
  startDelay?: number;
  /** Replaces the on-screen text, slot by slot (see ThemeOrganicTexts). */
  texts?: readonly string[];
}) => {
  const t = mergeTexts(texts, ThemeOrganicTexts);
    ...
    <AbsoluteFill style={{ background: "#faf8f5" }}>
      ...
      >
        ...
        >
          {t[1]}
          <br />
          <span style={{ fontWeight: 600 }}>{t[2]}</span>
        </div>
      </div>
    </AbsoluteFill>
  );
