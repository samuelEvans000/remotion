...
export const ThemeBohoTexts = [
  "FREE SPIRIT",
  "Bohemian",
  "Style"
] as const;

export const ThemeBoho = ({ startDelay = 0, texts }: {
  startDelay?: number;
  /** Replaces the on-screen text, slot by slot (see ThemeBohoTexts). */
  texts?: readonly string[];
}) => {
  const t = mergeTexts(texts, ThemeBohoTexts);
    ...
    <AbsoluteFill style={{ background: "#faf3e8" }}>
      ...
      >
        ...
        >
          {t[1]}
          <br />
          <span style={{ fontStyle: "italic" }}>{t[2]}</span>
        </div>
      </div>
