...
export const Phone: React.FC<{
  ...
  // When rendering, <ThreeCanvas> only draws when the frame number changes, but
  // useOffthreadVideoTexture delivers each frame's texture asynchronously after that.
  // Draw again once it arrives, otherwise the screen stays blank in rendered output.
  const advance = useThree((state) => state.advance);
  useEffect(() => {
    if (videoTexture) advance(performance.now());
  }, [advance, videoTexture]);
